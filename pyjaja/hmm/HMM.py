...
class HMM_state(Model_state):
	...
	def next_obs(self) -> str:
		"""
		Generate one observation according to the distribution described by `self.output_matrix`.
	...
	def next_state(self) -> int:
		"""
		Return one state according to the distribution described by the `self.next_matrix`.
	...
	def next(self) -> list:
		"""
		Return a state-observation pair according to the distributions described by `self.next_matrix` and `self.output_matrix`.
...
class HMM(Model):
	...
	def b(self,s: int, l: str) -> float:
		"""
		Return the probability of generating `l` in state `s`.
