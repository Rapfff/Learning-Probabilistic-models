...
for i in range( len(num_states) ): # Number of States
    ...
    for j in range( len(num_sequences) ): # Number of Sequnces 
        for k in range( num_tests ): # Tests with same parameters
            print( "States:[",i+1,"/",len(num_states),"] Sequences:[",j+1,"/",len(num_sequences),"] Tests:[",k+1,"/",num_tests,"]")
            ...
            res[i][j][k][2] = learned_model.logLikelihood( training_set ) - original_model.logLikelihood( training_set )
