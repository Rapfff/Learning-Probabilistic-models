		...
		for seq in range(len(self.sequences_sorted)):
			...
			if proba_seq != 0.0:
				for k in range(len(sequence_obs)):
					gamma_s1_k = alpha_matrix[s1][k] * beta_matrix[s1][k] / proba_seq
					
					den[self.actions.index(sequence_actions[k])] += gamma_s1_k * self.times[seq]
					if sequence_obs[k] == obs:
						num[self.actions.index(sequence_actions[k])] += alpha_matrix[s1][k]*self.h_g(s1,sequence_actions[k],s2,obs)*beta_matrix[s2][k+1] * self.times[seq] / proba_seq
		
				if loglikelihood != None:
					loglikelihood += log(proba_seq)
			prev_len = len(sequence_obs)
