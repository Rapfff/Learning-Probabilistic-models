...
def experiment(original_model, name, nb_states_small, nb_states_big,nb_experiments):
    ...
    for c in range(nb_experiments):
        print("Classic ",c+1,"/",nb_experiments," ",name,sep="")
        s = datetime.now()
    ...
    return ([[[mean(running_time[i]),stdev(running_time[i])] for i in range(2)],
            [[mean(quality[i]),stdev(quality[i])] for i in range(2)]])


running_time, quality = experiment(modelMC_REBER(),"REBER",3,7,5)
string1 = ""
string1 += "Model: REBER"+'\n'
string1 += "Average running time composition: "+str(running_time[0][0])+'\n'
string1 += "Average running time classical: "+str(running_time[1][0])+'\n'
string1 += "Std running time composition: "+str(running_time[0][1])+'\n'
string1 += "Std running time classical: "+str(running_time[1][1])+'\n'
string1 += "Average loglikelihood composition: "+str(quality[0][0])+'\n'
string1 += "Average loglikelihood classical: "+str(quality[1][0])+'\n'
string1 += "Std loglikelihood composition: "+str(quality[0][1])+'\n'
string1 += "Std loglikelihood classical: "+str(quality[1][1])+'\n'
string1 += '\n'
print(string1)

running_time, quality = experiment(modelMC_map(),"MAP",5,9,1)
string2 = ""
string2 += "Model: REBER"+'\n'
string2 += "Average running time composition: "+str(running_time[0][0])+'\n'
string2 += "Average running time classical: "+str(running_time[1][0])+'\n'
string2 += "Std running time composition: "+str(running_time[0][1])+'\n'
string2 += "Std running time classical: "+str(running_time[1][1])+'\n'
string2 += "Average loglikelihood composition: "+str(quality[0][0])+'\n'
string2 += "Average loglikelihood classical: "+str(quality[1][0])+'\n'
string2 += "Std loglikelihood composition: "+str(quality[0][1])+'\n'
string2 += "Std loglikelihood classical: "+str(quality[1][1])+'\n'
string2 += '\n'
print(string2)
