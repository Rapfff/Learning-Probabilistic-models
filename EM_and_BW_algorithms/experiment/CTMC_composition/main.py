...
saveSet(untimed_test_set_composition,"test_set_composition.txt")
...
timed_model1, timed_model2 = BW_CTMC_Composition(random1,random2).learn(timed_training_set,pp="Timed approach")
...
print()
_, untimed_model2 = BW_CTMC_Composition(original1,random2).learn(untimed_training_set,verbose=True,pp="Untimed approach",to_update=2)
untimed_model2.save("output/untimed_model2.txt")

print()
simple = BW_CTMC(parallelComposition(random1,random2)).learn(timed_training_set,output_file="output/simple.txt",pp="Simple approach",verbose=True)
...
simple = loadCTMC("output/simple.txt")
